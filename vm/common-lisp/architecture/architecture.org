...
* 記
** 引
   1. cicada-image 是磁盤中的一個路徑[而不是一個文件]
      路徑中的每個子路徑代表一個 cicada-section
      ".cicada-section" 是 cicada-section 的內容
      ".cicada-section-meta" 是 cicada-section 的屬性
   2. 我把 各個 cicada-image 都保存在 "cicada:image;" 這個路徑下
   3. cicada-memory 是一整塊線性的內存
      它被劃分爲各個 section
      ...
      這個姓代表了它所屬的 section
      沒有無姓的地址
   4. 可以通過把 cicada-image 中的一個 section
      加載到 cicada-memory 中
      而使用掉某些內存 來實現所希望的數據結構
   5. 多個 image 中的 section 可以被加載到同一個 cicada-memory 中
      只要 section 不重名即可
   6. 加載的過程中
   ...
   7. 同時 vm 中也設置了一些變量和函數
      來查詢對 cicada-memory 的使用情況
      比如 *data-section-record#cicada-memory* 這個全局變量
   8. 對 cicada-section 的 fetch & save 提供了一致的接口 
** 例
   1. vector-function-heap 是一個 section
      [將來還會有 list-function-heap 等等]
      其中有很多函數
      每個函數作爲 vector 的長度不一樣
   2. 比如 "cicada:source;core;" 路徑下的
      "english-core.cicada-source"
      就被編譯成一個 vector-function-heap
   3. 加載一個 vector-function-heap 之後
      把其中的一個 vector-function-body 的地址入 return-stack
      然後就可以啓動 vm 了
* cicada-image
** note
