#+TITLE: threaded code of cicada extensible virtual machine
#+AUTHOR: XIE Yuheng
...
** note
   ...
   2. 新增加的 所谓 "更好的编译语义"
      要求 在 call-compiled-phrase 之后
      应该把 Waiting-symbol-list 中
      由 compile-phrase 引入的等待中的 symbol 清除
      + 可以 利用 地址 的区别来做一个谓词
        要知道在 compile-phrase 中 Here 是不同的
      + 尽管在观念上我有点接受不了用 地址来做谓词这种行为
        但是 这有时候是无法避免的
        并且这也根本没有什么错
      + 要知道除非你去用纯粹的文本来形成对数据结构的描述
        否则 地址的差异一定会形成关键的语义的
...
** clear-waiting-symbol-list,for-phrase
   ...
   ;; 检测链表中的第二个元素 而不是第一个元素
   ;; 这样就可以在 检查到 phrase 的 buffer 中的地址时
   ;; 从中间消去链表的一个元素
   ;; 结束递归的条件是 发现 链表是 由单个元素组成的
   ;; 当链表不是由单个元素组成的 的时候
   ;; 递归作用以两种方式减少 链表的长度
   ;; 1. 不需要消去时 参数后移
   ;; 2. 需要消去时 消去一个元素
   ;; 链表的头元素始终是被处理过了的 (即 确定以需要被包含在链表内的)
   ;; 空链表和链表元素个数为 1 的情形
   ;; 在递归部分之外处理
   ;; 其实 不应该说是 递归之外
   ;; 因为这里是在用一个循环 来寻找
...
** sub-symbol-from-waiting-symbol-list
   #+begin_src fasm :tangle threaded-code.inc
   ;; 比对链表中的第二个元素 而不是第一个元素
   ;; 这样就可以在 比对成功时 从中间消去链表的一个元素
   ;; 结束递归的条件是 发现 链表是 由单个元素组成的
   ;; 当链表不是由单个元素组成的 的时候
   ;; 递归作用以两种方式减少 链表的长度
   ;; 1. 不需要消去时 参数后移
...
** meet-undefined-word
   ...
   def_verb "meet-undefined-word", meet_undefined_word
      ;; (* -- *)
      xx Message__undefined_word, print_string, cr
      xx Exit
...
** compile-word-description-to-here
   ...
   def_verb "compile-word-description-to-here", compile_word_description_to_here
      ...
      ;; xx Message__undefined_word, print_string
      ;; xx print_string, cr
      ;; xx Message__the_rest, print_string, cr
      ;; xx print_string, cr
      ;; xx Exit
   def_verb_end compile_word_description_to_here

   def_string "Message:the-rest", "   The rest of the word string to compile is:", Message__the_rest
   #+end_src
...
*** note
    ...
    5. >< 这里同样也需要清理 Waiting-symbol-list 吗 ???
    6. 没错
       所有的 定义之后马上就被调用的函数
       都需要清理 Waiting-symbol-list
    7. 只有对于 那些被编译到字典中的词
       把 symbol 保留存在 Waiting-symbol-list 中才有意义
*** implementation
    #+begin_src fasm :tangle threaded-code.inc
...
*** clear-waiting-symbol-list,for-define
    ...
    ;; 检测链表中的第二个元素 而不是第一个元素
    ;; 这样就可以在 检查到 define 的 buffer 中的地址时
    ;; 从中间消去链表的一个元素
    ;; 结束递归的条件是 发现 链表是 由单个元素组成的
    ;; 当链表不是由单个元素组成的 的时候
    ;; 递归作用以两种方式减少 链表的长度
...
** define-key-word
   ...
   def_verb "define-key-word", define_key_word
      ...
      xx   compile_word_description_to_here
      xx pop_lambda_stack
      ;; 这里 如果能够 在 Waiting_symbol_list 中找到 这个 词
      ;; 就应该报错
      ;; 因为此时在 用户在 使用一个 语法关键词 之前
      ;; 没有定义这个语法关键词
      ;; 如果 不处理这个 错误的话
      ;; 只有当 用户定义一个
      ;; 与 这个语法关键词 有 同样名字的 名词或动词
      ;; 的时候 这个 等待 被定义的 位置 才会 被补上
      ;; 我 暂时不处理这个 错误
      xx dup, set_size_of_word
