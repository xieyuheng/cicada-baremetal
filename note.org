...
1. 在 尝试以 统一的方式 处理 define 时遇到了困难
   即 当 define 看到 <pair-like> 的量时
   就应该 执行 try,white->grey
   而我不可能把 所依赖的所有的函数都转移到 汇编代码中
   这个问题 有两个解法:
   1) 用别的语言写一个 cicada 代码 到 汇编代码的编译器
      - 优点是 :: 整个代码都可以互相调用了
      - 缺点是 :: 要处理命名规则
                  还有 其他的 可能存在的 跟汇编代码中 symbol 的冲突
   2) 在 cicada 的 runtime 即 basic-REPL 中实现 编译语义的性质
      即等待 未定义的 变量和函数
      这样 "define" 在 cicada 代码中的位置就无所谓了
      - 优点是 :: 没有对其他语言的依赖
                  并且这个性状是早晚都要实现的
      - 缺点是 :: 并不是全部的代码都可以相互调用
2. 我选择第二个解法
3. 在解释器的开始 我必须有 define-function 能够使用
   并且 首先需要定义的函数就是 define
4. 为了在 runtime 中形成编译语义
   我必须把 对类型系统的处理 还有 hash-table 都转移到 汇编代码当中
   这样在 用 cicada 代码 来实现的就只是
   gc 还有 list-processing 还有 lambda-calculus 了
5. 把越来越多的函数转移到 汇编代码中让我感觉很不好
   但是 现在的 runtime 更加复杂了
   而 在有 能编译自身 的编译器出现之前
   我必须把 runtime 所需要的函数都放到 汇编代码中
   其实 没什么好犹豫的
----------------------------------
...
2. ><><>< 注意 ><><><
   这里要更改跟 GC 有关的代码
   整个 GC 都应该被重写了吗 ???
3. move "define" to assembly code
4. 新的 define 还有一个问题是
