...
* ==================================================
* TODO
  1. need to redesign the way to do test
     should tangle test to separated file
     for i do not have my org-mode now
  2. need to rethink the names about substring & string
  3. wodyli processing
     then digrap in cicada
  4. tab completion in the REPL
     (or in text editor)
  5. better basic interface of the OS
  ...
  10. cicada should design it's own abstractions of IO
      this can help cicada to be portable
  11. 在核心就直接使用hash-table来find
      但是list还是必要的
      因为这样就能保持对字典的灵活控制
  12. better interface for lambda
  13. memory allocation
  14. use hash-table to lookup dictionary
      then mutually recursive call
  15. flow-point number
  16. >< maybe substitute word to execute in lambda-body
  17. >< maybe syntax-check
  18. >< modern pattern-match  HOW ???
  19. better interface for debuger
  20. TAIL-CALL-OPTIMIZATION in apply lambda-body
  21. more protection on allocated memory
      maybe use paging
  22. cicada-assembler
  23. cicada-editor
  24. APL-like array-processing
  25. needs better sexp-reader&write
      design a better way
